namespace Aufgabe5 {
	...
	function init(_event: Event): void {
		...
		for (let i: number = 0; i<bienen; i++) {
			let b: Bee = { beex: 640, beey: 410, color: "gold", bluewings: false};
	...
	function drawBee(_x: number, _y: number, _color: string, _bluewings: boolean): void {
			// Korpus
		crc2.beginPath();
		...
		let gradient : CanvasGradient = crc2.createLinearGradient(_x-16,0,_x+16,0);
			...
			gradient.addColorStop(0.199, _color);
			gradient.addColorStop(0.2, "black");
			gradient.addColorStop(0.399, "black");
			gradient.addColorStop(0.4, _color);
			gradient.addColorStop(0.599, _color);
			gradient.addColorStop(0.6, "black");
			gradient.addColorStop(0.799, "black");
			gradient.addColorStop(0.8, _color);
			gradient.addColorStop(1, _color);
	...
	function animate(_width: number, _height: number): void {
		...
		for (let i: number = 0; i < bees.length; i++) { // Zufällige Bewegung der Bienen
			let b: Bee = bees[i];
            b.beex += Math.floor(Math.random() * 6) - 3;
            b.beey += Math.random() * 4 - 2;
            ...
            drawBee(b.beex, b.beey, b.color, b.bluewings);
        }
